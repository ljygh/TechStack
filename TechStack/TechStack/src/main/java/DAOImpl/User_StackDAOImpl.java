...
public class User_StackDAOImpl implements User_StackDAO {
    ...
    public List<Stack> SearchStackByName(int uid, String sname) throws SQLException {
        List<Stack> stackList = new LinkedList<>();
        String sql = "select * from stack where sid in (select sid from user_stack where uid = ?) and sname like ?";
        PreparedStatement statement = conn.prepareStatement(sql);
        statement.setInt(1, uid);
        statement.setString(2, "%" + sname + "%");
        ResultSet resultSet = statement.executeQuery();
        while (resultSet.next()){
            int sid = resultSet.getInt("sid");
            String stackName = resultSet.getString("sname");
            String description = resultSet.getString("description");
            stackList.add(new Stack(sid, stackName, description));
        }
        return stackList;
    }
}
